...
local function fitTerrainToBuilding(objId, terrainId)
    ...
    ry = ry or 0
    -- GE rotates clockwise around Y (East→South for positive angle).
    -- Negating sinR converts to that convention.
    local cosR, sinR = math.cos(ry), -math.sin(ry)
    local floorY = cy - sinkM
    ...
    print(string.format("  Floor Y  : %.3f   Margin: %.1f m   Step: %.2f m",
        floorY, marginM, gridStep))
    ...
    while x <= x1 do
        ...
        while z <= z1 do
            ...
            if inside then
                -- Uniform floor inside — eliminates all steps and poke-through
                setTerrainHeightAtWorldPos(terrainId, x, rayY, z, floorY)
                nInner = nInner + 1
            elseif dist <= marginM then
                -- Feathered blend outside — only ever lowers, never raises
