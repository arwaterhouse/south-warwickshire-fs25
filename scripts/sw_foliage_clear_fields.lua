...
-- Name: SW Foliage Clear Fields
-- Description: Removes ALL decoration foliage from terrain that matches
--              known field / cultivated / road texture signatures.
--              Run this to undo foliage that was incorrectly painted on fields.
--
-- Two modes:
--   ERASE BY TEXTURE — scans the whole map partition by partition and erases
--                      foliage wherever the terrain RGBA matches field textures.
--                      Uses exact signatures read from your map.i3d.
--
--   ERASE BRUSH      — select a node in GE, click Erase Here to clear a
--                      circular area manually (for spot fixes).
--
-- Textures that will be cleared (from map.i3d attributes):
--   MUD TRACKS       R=0.062 G=0.044 B=0.027 W=1.0  (freshly cultivated / tyre tracks)
--   MUD DARK         R=0.071 G=0.055 B=0.043 W=1.0  (dark ploughed soil)
--   MUD GRAVEL       R=0.176 G=0.153 B=0.133 W=1.0  (farm track / gateway)
--   MUD LIGHT        R=0.137 G=0.114 B=0.078 W=1.0  (light mud)
--   MUD PEBBLES      R=0.067 G=0.051 B=0.039 W=1.0  (pebbly mud)
--   MUD DARK PATCHY  R=0.078 G=0.067 B=0.043 W=1.0  (patchy mud)
--   MUD LEAVES       R=0.082 G=0.075 B=0.027 W=1.0
--   MUD TRACKS 2     R=0.137 G=0.098 B=0.075 W=1.0
--   ASPHALT / ROAD   R=0.071 G=0.063 B=0.063 W=0.0  (tarmac — no foliage on roads)
...
--   ROCK FLOOR TILES R=0.208 G=0.184 B=0.165 W=0.0
--
-- Hide: no
-- AlwaysLoaded: no
...
-- ── Field / hard-surface texture signatures ───────────────────────────────────
...
-- Erase all foliage planes over a rectangular world-space area.
-- One DensityMapModifier call per plane (NOT per grid point) — this matches
-- the approach used in sw_foliage_brush.lua and is orders of magnitude faster
-- than the previous per-point approach which was creating ~100k+ objects per
-- partition and causing GE to silently drop most of the work.
...
-- ── Partition scan (erase by texture) ────────────────────────────────────────
...
local function runNextPartition()
    ...
    -- Scan texture row by row (Z outer, X inner).
    -- Consecutive matching X positions on the same Z row are merged into a
    -- single span so we call eraseRect once per span rather than once per point.
    -- This keeps the total number of DensityMapModifier objects low and fast.
    local erased = 0
    local texMatches = 0
    ...
    while z <= zEnd do
        ...
        while x <= xEnd + scanSpacing * 0.1 do   -- small epsilon avoids float drift
            ...
            if inSpan then
                ...
                if spanX1 then
                    -- Close span: erase the rectangle covering [spanX1 .. x-scanSpacing] on this Z row
...
local function eraseHere()
    ...
    -- Single eraseRect call covering the entire brush square — same approach as
    -- sw_foliage_brush.lua which is confirmed working. No per-point loop needed.
